...
package body Control is
   ...
   procedure wait(controller: in out CONTROLLER_TYPE) is
   begin
      Put_Line(" >>WAIT 1");
   ...
   begin
      ...
      Put_Line("=>notify target TRANSFER");
      Signals.Notify(target.run);
      wait(controller);
   ...
   begin
      ...
      Put_Line("=>notify invoker");
      Signals.Notify(invoker.run);
      Put_Line("=>SUSPEND WAIT");
      wait(controller);
      Put_Line("<=SUSPEND WAIT");
   end Suspend;
